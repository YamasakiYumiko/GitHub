...
<head>
	...
	<title><?php __('CalendarNote'); ?>	</title>
	<?php
		echo $html->css('style');
		if (Configure::read() > 0) {
...
<body>
	<div id="container">
		<div id="header">
			<h1><?php echo $html->link(__('CalendarNote', true), '/'); ?></h1>
			<p class='userprofile'><?php 
				if($loginUser != null) {
			...
			<?php
			if($loginUser != null) {
				echo $this->element('action_menu' ,array("cache" => array('time'=> "+1 day",'key'=>$loginUser['User']['id'])));
			}
			?>
		</div>
		<div id="content">
