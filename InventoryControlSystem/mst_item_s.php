...
// 	$order_by = (string) @$_POST['order_by'];
	...
	//				,"totalization_cd2"
	//				,"totalization_cd3"
	//				,"packing_control_unnecessary_flg"
	//				,"lot_control_flg"
	//				,"active_span_control_flg"
	//				,"active_span_convert_kubun_cd"
	//				,"unit_flg"
	//				,"salable_rank"
	//				,"inventories_evaluation_standard"
	//				,"consumption_tax_kubun_cd"
	//				,"consumption_tax_rate_kubun_cd"
	//				,"unit_price_calc_kubun_cd"
	//				,"lot_stock_evaluation_flg"
	//				,"selling_cost_of_unit_price_calc_kubun_cd"
...
QUERY_EOD;
	...
	LEFT JOIN mst_obic_custmer_item_price as m1 ON
		m1.item_cd = m.item_cd
		AND (
			('$ymd1' BETWEEN m1.active_span_st AND m1.active_span_ed) OR
			('$ymd1' >= m1.active_span_st AND m1.active_span_ed IS NULL)
		)
	WHERE
		TRUE
QUERY_EOD;
			...
			<div class="searchbox">
				...
				<form method="POST" action="">
					<table style="border-style: none">
						<tr>
							<th>商品CD</th>
							<td><input type="text" name="mst_cd1" value="<?php echo $mst_cd1; ?>" maxlength="7" onkeypress="return EnterFocusCD(this, 'mst_cd2', 'mst_cd1', 'D', 7, false)" onblur="return FocusCD(this, 'mst_cd2', 'mst_cd1', 'D', 7, false)" />
								~
								<input type="text" name="mst_cd2" value="<?php echo $mst_cd2; ?>" maxlength="7" onkeypress="return EnterFocusCD(this, 'item_name1', 'mst_cd2', 'D', 7, false)" onblur="return FocusCD(this, 'item_name1', 'mst_cd2', 'D', 7, false)" /></td>
							<th>商品名</th>
						...
						if (! $res) {
							...
							for($count = 0; $count < count ( $m_arr2 ); $count ++) {
								if ($m_arr2 [$count] == "") {
									...
									echo "<th>" . $m_arr2 [$count] . "<br><button type=\"submit\" name=\"order_by\" value=\"CASE WHEN m.item_cd~ '^D[0-9]{1,}' THEN 'D' || TO_CHAR(CAST(REPLACE( m.item_cd ,'D' ,'' ) AS INTEGER),'FM000000') ELSE m.item_cd END ASC\">↓</button><button type=\"submit\" name=\"order_by\" value=\"CASE WHEN m.item_cd~ '^D[0-9]{1,}' THEN 'D' || TO_CHAR(CAST(REPLACE( m.item_cd ,'D' ,'' ) AS INTEGER),'FM000000') ELSE m.item_cd END DESC\">↑</button></th>";
								} else if ($m_arr1 [$count] == "selling_price") {
