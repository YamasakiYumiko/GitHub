...
QUERY_EOD;
	...
	SELECT
		...
		p.warehouse_cd AS warehause_cd,
		mw.warehause_name AS warehause_name,
		...
		TO_CHAR(p.created_timestamp, 'YYYY-MM-DD HH24:MI:SS') AS created_timestamp,
		p.modified_cd AS modified_cd,
		TO_CHAR(p.modified_timestamp, 'YYYY-MM-DD HH24:MI:SS') AS modified_timestamp,
		...
		pd.warehouse_cd AS d_detail_warehause_cd,
		...
		CAST(pd.purchase_qty AS INTEGER) AS d_quantity,
		REPLACE(TO_CHAR(pd.purchase_cost, '9999999990.99'), '.00', '&nbsp;&nbsp;&nbsp;') AS d_supplier_price,
		CAST(pd.purchase_amount AS INTEGER) AS d_supplier_amount,
		...
		TO_CHAR(pd.created_timestamp, 'YYYY-MM-DD HH24:MI:SS') AS d_created_timestamp,
		pd.modified_cd AS d_modified_cd,
		TO_CHAR(pd.modified_timestamp, 'YYYY-MM-DD HH24:MI:SS') AS d_modified_timestamp
QUERY_EOD;
	...
	LEFT JOIN mst_obic_warehause AS mw ON
		mw.warehause_cd = p.warehouse_cd
	LEFT JOIN (
		SELECT
			m1.staff_cd,
			m1.staff_name
				...
				<form method="POST" action="" name="form1">
					<table style="border-style: none">
						<tr>
							...
							<th>仕入先CD</th>
							<td>
								<input type="text" name="supplier_cd1"
...
QUERY_EOD;
			...
			<div class="resultbox">
				<form method="POST" action="">
					...
					<input type="hidden" name="purchase_ymd1"
						value="<?php echo "$purchase_ymd1"; ?>" />
					<input type="hidden" name="purchase_ymd2"
						value="<?php echo "$purchase_ymd2"; ?>" />
...
//							echo "<th>伝票番号<br><button type=\"submit\" name=\"order_by\" value=\"pd.purchase_slip_no\">↓</button><button type=\"submit\" name=\"order_by\" value=\"pd.purchase_slip_no DESC\">↑</button></th>";
							echo "<th>明細行番号<br><button type=\"submit\" name=\"order_by\" value=\"pd.purchase_detail_no\">↓</button><button type=\"submit\" name=\"order_by\" value=\"pd.purchase_detail_no DESC\">↑</button></th>";
							...
							echo "<th>規格<br><button type=\"submit\" name=\"order_by\" value=\"pd.model_name\">↓</button><button type=\"submit\" name=\"order_by\" value=\"pd.model_name DESC\">↑</button></th>";
							...
							echo "<th>明細数量<br><button type=\"submit\" name=\"order_by\" value=\"pd.purchase_qty\">↓</button><button type=\"submit\" name=\"order_by\" value=\"pd.purchase_qty DESC\">↑</button></th>";
							...
							while ( $row = pg_fetch_assoc ( $res ) ) {
								...
								$warehouse_cd = (string) $row['warehause_cd'];
								$warehouse_name = (string) $row['warehause_name'];
								...
								$d_detail_warehause_cd = (string) $row['d_detail_warehause_cd'];
...
//								echo "<td>$d_slip_no</td>";
								...
								echo "<td>$d_detail_warehause_cd</td>";
