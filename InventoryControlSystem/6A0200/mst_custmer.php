<?php
	...
	if ($order_by == null) {
		$order_by = "m.custmer_cd";
	}

	$mst_cd1 = (string) @$_POST['mst_cd1'];
	$mst_cd2 = (string) @$_POST['mst_cd2'];

	$custmer_short_name1 = (string) @$_POST['custmer_short_name1'];
...
QUERY_EOD;
	...
	// 取引先名
	if ($custmer_short_name1 != '') {
		$query = $query . <<<QUERY_EOD
		AND m.custmer_short_name LIKE '%$custmer_short_name1%'
QUERY_EOD;
	}
?>
					...
					<table style="border-style: none">
						<tr>
							...
							<td><input type="text" name="mst_cd1" value="<?php echo $mst_cd1; ?>" maxlength="10" onkeypress="return EnterFocusCD(this, 'mst_cd2', 'mst_cd1', '', 10, true)" onblur="return FocusCD(this, 'mst_cd2', 'mst_cd1', '', 10, true)" />
								~
								<input type="text" name="mst_cd2" value="<?php echo $mst_cd2; ?>" maxlength="10" onkeypress="return EnterFocusCD(this, 'custmer_short_name1', 'mst_cd2', '', 10, true)" onblur="return FocusCD(this, 'custmer_short_name1', 'mst_cd2', '', 10, true)" /></td>
							<th>得意先名</th>
							<td><input type="text" name="custmer_short_name1" value="<?php echo $custmer_short_name1; ?>" onkeypress="return EnterFocus(this, 'interval')" /></td>
						</tr>
						<tr>
							<th>表示件数</th>
						...
						if (! $res) {
							...
							for ($count=0; $count < count($m_arr2); $count++)
							{
								if ($m_arr2[$count] == "") {
									...
									echo "<th>".$m_arr2[$count]."<br><button type=\"submit\" name=\"order_by\" value=\"m1."."staff_name"."\">↓</button><button type=\"submit\" name=\"order_by\" value=\"m1."."staff_name"." DESC\">↑</button></th>";
								} else {
