<?php
	...
	FROM mst_obic_payment_close_date
QUERY_EOD;
...
QUERY_EOD;
	...
	$query = <<<QUERY_EOD
	FROM mst_obic_payment_close_date as m
...
QUERY_EOD;
	...
	if ($payee_name1 != '') {
		$query = $query . <<<QUERY_EOD
		AND m1.payee_name LIKE '%$payee_name1%'
QUERY_EOD;
	}
?>
						...
						if (! $res) {
							...
							for ($count=0; $count < count($m_arr2); $count++)
							{
								if ($m_arr2[$count] == "") {
									...
									echo "<th nowrap>".$m_arr2[$count]."<br><button type=\"submit\" name=\"order_by\" value=\"m1."."payee_name"."\">↓</button><button type=\"submit\" name=\"order_by\" value=\"m1."."payee_name"." DESC\">↑</button></th>";
								} else {
