...
QUERY_EOD;
	...
	SELECT
		...
		mi.jan_cd AS jan_cd,
		CAST(tcs.current_quantity AS INTEGER) AS current_quantity
QUERY_EOD;
	...
	LEFT JOIN mst_obic_warehause AS mw ON
		mw.warehause_cd = tcs.warehause_cd
	WHERE
		TRUE
...
QUERY_EOD;
			...
			<div class="resultbox">
				<form method="POST" action="">
					...
					<input type="hidden" name="stock0_flg" value="<?php echo "$stock0_flg"; ?>" />
					<input type="hidden" name="warehouse_cd1" value="<?php echo "$warehouse_cd1"; ?>" />
					<input type="hidden" name="item_kind_category_cd1" value="<?php echo "$item_kind_category_cd1"; ?>" />
					<input type="hidden" name="item_kind_cd1" value="<?php echo "$item_kind_cd1"; ?>" />
					<input type="hidden" name="item_cd1" value="<?php echo "$item_cd1"; ?>" />
					<input type="hidden" name="item_cd2" value="<?php echo "$item_cd2"; ?>" />
					<input type="hidden" name="item_name1" value="<?php echo "$item_name1"; ?>" />
					<?php
						foreach ( $conn_array as $key => $value ) {
							...
							if (! $res) {
								...
								echo "<tr>";
								echo "<th></th>";
								echo "<th>倉庫CD<br><button type=\"submit\" name=\"order_by\" value=\"tcs.warehause_cd\">↓</button><button type=\"submit\" name=\"order_by\" value=\"tcs.warehause_cd DESC\">↑</button></th>";
								...
								echo "<th>品種カテゴリCD<br><button type=\"submit\" name=\"order_by\" value=\"mikc.item_kind_category_cd\">↓</button><button type=\"submit\" name=\"order_by\" value=\"mikc.item_kind_category_cd DESC\">↑</button></th>";
								echo "<th>品種カテゴリ名<br><button type=\"submit\" name=\"order_by\" value=\"mikc.item_kind_category_name\">↓</button><button type=\"submit\" name=\"order_by\" value=\"mikc.item_kind_category_name DESC\">↑</button></th>";
								echo "<th>品種CD<br><button type=\"submit\" name=\"order_by\" value=\"mik.item_kind_cd\">↓</button><button type=\"submit\" name=\"order_by\" value=\"mik.item_kind_cd DESC\">↑</button></th>";
								echo "<th>品種名<br><button type=\"submit\" name=\"order_by\" value=\"mik.item_kind_name\">↓</button><button type=\"submit\" name=\"order_by\" value=\"mik.item_kind_name DESC\">↑</button></th>";
								// 2016-05-20 kinoshita
								// echo "<th>商品CD<br><button type=\"submit\" name=\"order_by\" value=\"tcs.item_cd\">↓</button><button type=\"submit\" name=\"order_by\" value=\"tcs.item_cd DESC\">↑</button></th>";
								echo "<th>商品CD<br><button type=\"submit\" name=\"order_by\" value=\"CASE WHEN tcs.item_cd~ '^D[0-9]{1,}' THEN 'D' || TO_CHAR(CAST(REPLACE( tcs.item_cd ,'D' ,'' ) AS INTEGER),'FM000000') ELSE tcs.item_cd END\">↓</button><button type=\"submit\" name=\"order_by\" value=\"CASE WHEN tcs.item_cd~ '^D[0-9]{1,}' THEN 'D' || TO_CHAR(CAST(REPLACE( tcs.item_cd ,'D' ,'' ) AS INTEGER),'FM000000') ELSE tcs.item_cd END DESC\">↑</button></th>";
								// 2016-05-20 kinoshita END
								...
								echo "<th>現在庫数<br><button type=\"submit\" name=\"order_by\" value=\"tcs.current_quantity\">↓</button><button type=\"submit\" name=\"order_by\" value=\"tcs.current_quantity DESC\">↑</button></th>";
								echo "</tr>";
