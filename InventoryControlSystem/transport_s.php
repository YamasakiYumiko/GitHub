...
QUERY_EOD;
	...
	SELECT
		...
		t.shipping_warehouse_cd AS out_warehouse_cd,
		mos.shop_short_name AS out_warehouse_name,
		t.arrival_office_cd AS in_office_cd,
		t.arrival_warehouse_cd AS in_warehouse_cd,
		mis.shop_short_name AS in_warehouse_name,
		...
		TO_CHAR(CAST(td.transfer_qty AS INTEGER), 'FM9,999,999,990') AS d_quantity,
		td.remarks AS d_detail_summary,
		td.created_cd AS d_created_cd,
		td.created_timestamp AS d_created_timestamp,
		td.modified_cd AS d_modified_cd,
		td.modified_timestamp AS d_modified_timestamp
QUERY_EOD;
	...
	LEFT JOIN mst_shop_for_tool AS mos ON
		mos.dept_cd = t.shipping_warehouse_cd
	LEFT JOIN mst_shop_for_tool AS mis ON
		mis.dept_cd = t.arrival_warehouse_cd
...
<!--
								...
								onkeypress="return EnterFocusCD(this, 'slip_no2', 'slip_no1', 'I', 12, true)"
								onblur="return FocusCD(this, 'slip_no2', 'slip_no1', 'I', 12, true)" />
								~ <input type="text" name="slip_no2"
								value="<?php //echo $slip_no2; ?>" maxlength="12"
								onkeypress="return EnterFocusCD(this, 'transport_kubun_cd1', 'slip_no2', 'I', 12, true)"
								onblur="return FocusCD(this, 'transport_kubun_cd1', 'slip_no2', 'I', 12, true)" />
-->


								<input type="text" name="slip_no1" value="<?php echo $slip_no1; ?>" maxlength="12" onkeypress="return EnterFocus(this, 'slip_no2')" />
								~
								<input type="text" name="slip_no2" value="<?php echo $slip_no2; ?>" maxlength="12" onkeypress="return EnterFocus(this, 'item_cd1')" />
...
QUERY_EOD;
						...
						if (count ( $arr_rnj1 ) == 0) {
							$query .= " AND (t.shipping_warehouse_cd = '$warehouse_cd1' OR t.arrival_warehouse_cd = '$warehouse_cd2')";
						} else if ($warehouse_cd1 == "" and $warehouse_cd2 == "") {
							$query .= " AND FALSE";
							/*
							 * $query .= " AND (t.shipping_warehouse_cd IN (";
							 *
							 ...
							 * }
							 *
							 * $query .= "'') OR t.arrival_warehouse_cd IN (";
							 *
							 * foreach($arr_rnj1 as $val) {
							 * if ($val != "") {
							 * $query .= "'".$val."',";
							...
							$query .= " AND t.shipping_warehouse_cd = '$warehouse_cd1'";
						} else if ($warehouse_cd1 == "" and $warehouse_cd2 != "") {
							$query .= " AND t.arrival_warehouse_cd = '$warehouse_cd2'";
						} else {
							$query .= " AND t.shipping_warehouse_cd = '$warehouse_cd1' AND t.arrival_warehouse_cd = '$warehouse_cd2'";
						}
					}
			...
			<div class="resultbox">
				...
				<form method="POST" action="">
					...
					<input type="hidden" name="interval"
						...
						name="transport_kubun_cd1"
						value="<?php echo "$transport_kubun_cd1"; ?>" /> <input
						type="hidden" name="warehouse_cd1"
						value="<?php echo "$warehouse_cd1"; ?>" /> <input type="hidden"
						name="warehouse_cd2" value="<?php echo "$warehouse_cd2"; ?>" /> <input
						type="hidden" name="transport_ymd1"
						value="<?php echo "$transport_ymd1"; ?>" /> <input type="hidden"
						name="transport_ymd2" value="<?php echo "$transport_ymd2"; ?>" />
					<input type="hidden" name="slip_no1"
						value="<?php echo "$slip_no1"; ?>" /> <input type="hidden"
						name="slip_no2" value="<?php echo "$slip_no2"; ?>" /> <input
						type="hidden" name="item_cd1" value="<?php echo "$item_cd1"; ?>" />
					<input type="hidden" name="item_cd2"
						value="<?php echo "$item_cd2"; ?>" /> <input type="hidden"
						name="item_name1" value="<?php echo "$item_name1"; ?>" />
					...
					foreach ( $conn_array as $key => $value ) {
						...
						if (! $res) {
							...
							echo "<th>出庫倉庫名<br><button type=\"submit\" name=\"order_by\" value=\"mos.shop_short_name\">↓</button><button type=\"submit\" name=\"order_by\" value=\"mos.shop_short_name DESC\">↑</button></th>";
							...
							echo "<th>入庫倉庫名<br><button type=\"submit\" name=\"order_by\" value=\"mis.shop_short_name\">↓</button><button type=\"submit\" name=\"order_by\" value=\"mis.shop_short_name DESC\">↑</button></th>";
							...
							echo "<th>行<br><button type=\"submit\" name=\"order_by\" value=\"td.transfer_detail_no\">↓</button><button type=\"submit\" name=\"order_by\" value=\"td.transfer_detail_no DESC\">↑</button></th>";
							...
							echo "<th>商品名補足<br><button type=\"submit\" name=\"order_by\" value=\"td.model_name\">↓</button><button type=\"submit\" name=\"order_by\" value=\"td.model_name DESC\">↑</button></th>";
							...
							echo "<th>移動数<br><button type=\"submit\" name=\"order_by\" value=\"td.transfer_qty\">↓</button><button type=\"submit\" name=\"order_by\" value=\"td.transfer_qty DESC\">↑</button></th>";
							...
							while ( $row = pg_fetch_assoc ( $res ) ) {
								...
								$out_warehouse_cd = ( string ) $row ['out_warehouse_cd'];
								$out_warehouse_name = ( string ) $row ['out_warehouse_name'];
								$in_office_cd = ( string ) $row ['in_office_cd'];
								$in_warehouse_cd = ( string ) $row ['in_warehouse_cd'];
								$in_warehouse_name = ( string ) $row ['in_warehouse_name'];
								...
								// echo "<td>$out_warehouse_cd</td>";
								echo "<td>$out_warehouse_name</td>";
								// echo "<td>$in_office_cd</td>";
								// echo "<td>$in_warehouse_cd</td>";
								echo "<td>$in_warehouse_name</td>";
